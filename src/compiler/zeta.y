...
Exp: Exp QMARK Exp COLON Exp { auto e = std::make_unique<ConditionalExp>(); SET_POS(e, @$); e->cond = std::move($1); e->thenBranch = std::move($3); e->elseBranch = std::move($5); $$ = tryExpFold(std::move(e)); }
    ...
    | Exp DOT ID LP ArgList RP { auto e = std::make_unique<MethodCallExp>(); SET_POS(e, @$); e->caller = std::move($1); e->funcName = std::move($3); e->args = std::move($5); $$ = std::move(e); }
    | Exp DOT ID LP RP { auto e = std::make_unique<MethodCallExp>(); SET_POS(e, @$); e->caller = std::move($1); e->funcName = std::move($3); $$ = std::move(e); }
    | Exp LP ArgList RP { auto e = std::make_unique<CallExp>(); SET_POS(e, @$); e->callee = std::move($1); e->args = std::move($3); $$ = std::move(e); }
    | Exp LP RP { auto e = std::make_unique<CallExp>(); SET_POS(e, @$); e->callee = std::move($1); $$ = std::move(e); }
